...
if [[ "$MODE" == "reality" ]]; then
    ...
    generate_certs
    if [[ "$MODE" == "hy2" ]]; then
        UDP_JSON='{ "type": "hysteria2", "tag": "hy2-in", "listen": "'$LADDR'", "listen_port": '$up', "users": [{ "password": "'$pass'" }], "tls": { "enabled": true, "certificate_path": "'$CERT_DIR'/cert.pem", "key_path": "'$CERT_DIR'/privkey.pem", "alpn": ["h3"] } }'
    else
        UDP_JSON='{ "type": "tuic", "tag": "tuic-in", "listen": "'$LADDR'", "listen_port": '$up', "users": [{ "uuid": "'$uuid'", "password": "'$pass'" }], "congestion_control": "bbr", "tls": { "enabled": true, "certificate_path": "'$CERT_DIR'/cert.pem", "key_path": "'$CERT_DIR'/privkey.pem", "alpn": ["h3"] } }'
    fi
